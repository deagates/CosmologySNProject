...
def nuisance_draw():
    alpha=ramdon.random(20,)
    beta=ramdon.random(2,4)
    delta_m=random.random(-1,0)
    # for gaussian random.gauss(mu, sigma)
    return alpha, beta, delta_m
...
def M(galax_stell_mass,delta_m):
    
    n = range(data_length-1)
    for i in n:
        if galax_stell_mass<10**10:
            f=M_prime
        else:
            f=M_prime+mass,delta_m
        M.append(f)
    return M
...
def likelihood_draw_mini(zhel_data,zcmb_data,mu_hat):
    om,ol,ok = omega_draw()
    mu = mu_model_calc(zhel_data,zcmb_data,om,ok,ol)
    P = likelihood_calc(mu_hat, mu)
    return P, mu, om, ol, ok
    
def step_mcmc_mini(zhel_data,zcmb_data,mu_hat) :
    P_old_mini, mu, om, ol, ok = likelihood_draw_mini(zhel_data,zcmb_data,mu_hat)
    for i in n:
        P_new_mini, mu, om, ol, ok = likelihood_draw_mini(zhel_data,zcmb_data,m_B_data,x1_data,c_data,galax_stell_mass)
        r=P_new_mini/P_old_mini;
        if r>1:
            w_mini=w_mini+1;
        else:
            w_mini=0;
            P_old_mini=P_new_mini;
    return om, ol, ok, w_mini, P_new_mini, mu
    
  
def likelihood_draw(zhel_data,zcmb_data,m_B_data,x1_data,c_data,galax_stell_mass)):
    alpha, beta, delta_m = nuisance_draw()
    M(galax_stell_mass,delta_m)
    #print "calculating estimated mu"
    mu_hat = mu_data_calc(m_B_data,x1_data,c_data,M,alpha,beta)
    om, ol, ok, w_mini, P_new_mini, mu=step_mcmc_mini(zhel_data,zcmb_data,mu_hat)
    P = P_new_mini
    return P, mu, mu_hat, om, ol, ok, alpha, beta, delta_m
...
def step_mcmc(single_run_length,zhel_data,zcmb_data,m_B_data,x1_data,c_data):
    ...
    #initialize 
    P_old, mu, mu_hat, om, ol, ok,alpha, beta, delta_m = likelihood_draw(zhel_data,zcmb_data,m_B_data,x1_data,c_data,galax_stell_mass);
#    print P_old, mu, mu_hat, om, ol, ok
    w=0
    n = range(single_run_length)
    for i in n:
        P_new, mu, mu_hat, om, ol, ok,alpha, beta, delta_m = likelihood_draw(zhel_data,zcmb_data,m_B_data,x1_data,c_data,galax_stell_mass)
        r=P_new/P_old;
        if r>1:
            w=w+1;
        else:
            w=0;
            P_old=P_new;
    return om, ol, ok,alpha, beta, delta_m w, P_new
